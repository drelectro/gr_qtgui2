  ...
  : QwtPlot(parent)
{
  ...
  // MJC set up Center Frequency Marker
...
FrequencyDisplayPlot::replot()
{
  ...
  _markerCF->setXValue((_startFrequency + _stopFrequency)/2);
  //_markerCF->setYValue(0);
...
FrequencyDisplayPlot::_resetXAxisPoints()
{
  ...
  //fprintf(stderr,"_resetXAxisPoints %f %f %d\n", _startFrequency, _stopFrequency, _numPoints);
  ...
  // MJC
  // Resetting the zoomer with the same start and stop frequency as was prevoiusly set
  // Seems to cause the zoomer to set the left/start frequency to zero.
  // Rather than digging into the Zoomer code I've taken the 'Dr Dr It hurts when I do this approach' for now.
  //
  if(fabs(zbase.left() - _startFrequency) > 0.000001 ||
     fabs(zbase.right() - _stopFrequency) > 0.000001){
