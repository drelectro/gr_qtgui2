                                     ...
                                     const bool showform)
{
  ...
  if(!_windowOpennedFlag){
    ...
    _spectrumDisplayWidget->ToggleTabFrequency(frequency);
    _spectrumDisplayWidget->ToggleTabWaterfall(waterfall);
    _spectrumDisplayWidget->ToggleTabTime(time);
    _spectrumDisplayWidget->ToggleTabConstellation(constellation);

    _windowOpennedFlag = true;

    _spectrumDisplayWidget->setSystem(this, _dataPoints, _fftSize);
  ...
  qApp->postEvent(_spectrumDisplayWidget,
  ...
  if(GetWindowOpenFlag()) {
    qApp->postEvent(_spectrumDisplayWidget,
		    new SpectrumFrequencyRangeEvent(_centerFrequency, 
						    _startFrequency, 
						    _stopFrequency));
    qApp->postEvent(_spectrumDisplayWidget, new SpectrumWindowResetEvent());
  }
  _droppedEntriesCount = 0;
				    ...
				    const double stopFreq)
{
  ...
  _spectrumDisplayWidget->SetFrequencyRange(_centerFrequency,
					  _startFrequency,
					  _stopFrequency);
  //_windowStateLock->Unlock();
}
...
{
  return (QWidget*)_spectrumDisplayWidget;
}

void
SpectrumGUIClass::SetTimeDomainAxis(double min, double max)
{
  _spectrumDisplayWidget->SetTimeDomainAxis(min, max);
}
...
{
  _spectrumDisplayWidget->SetConstellationAxis(xmin, xmax, ymin, ymax);

}

void
SpectrumGUIClass::SetConstellationPenSize(int size){
  _spectrumDisplayWidget->SetConstellationPenSize(size);
}
...
{
  _spectrumDisplayWidget->SetFrequencyAxis(min, max);
}
